...
if backend.lower() == 'mxnet':
    ...
    from .pytorch.cus_external_emb import CustomExternalEmbedding
...
class KEModel(object):
    ...
    def __init__(self, args, model_name, n_entities, n_relations, hidden_dim, gamma,
        ...
        if USE_MY_EMB:
            self.entity_emb = CustomExternalEmbedding(args, n_entities, entity_dim,
                                                "full_emb")
        else:
        ...
        if not self.strict_rel_part and not self.soft_rel_part:
            if False and USE_MY_EMB:
                self.relation_emb = CustomExternalEmbedding(args, n_relations, rel_dim,
                                                    "relation_emb")
            else:
            ...
            if False and USE_MY_EMB:
                self.global_relation_emb = CustomExternalEmbedding(args, n_relations, rel_dim, "global_relation_emb")
            else:
        ...
        if model_name == 'TransE' or model_name == 'TransE_l2':
            ...
            if False and USE_MY_EMB:
                projection_emb = CustomExternalEmbedding(args,
                                                n_relations,
    ...
    def log_tensor(self, keys):
        rank = dist.get_rank()
        if not hasattr(self, "fd"):
            self.fd = open(f"/home/xieminhui/RecStore/src/framework_adapters/torch/kg/datasets/FB15k_keys_rank{rank}", "w")
        self.fd.write(f'{" ".join([str(each) for each in keys.tolist()])}\n')
    ...
    def forward(self, pos_g, neg_g, gpu_id=-1):
        ...
        with xmh_nvtx_range(f"others", gpu_id==0):
    ...
    def prepare_per_worker_process(self):
        self.entity_emb.prepare_per_worker_process()
        self.relation_emb.prepare_per_worker_process()
        
        if hasattr(self, "global_relation_emb"):
            self.global_relation_emb.prepare_per_worker_process()
    ...
    def push_gradient(self, client):
        ...
        with th.no_grad():
            for each_idx, each_grad in self.entity_emb.emb.get_grad():
                client.push(name='entity_emb',
                            id_tensor=each_idx,
                            data_tensor=each_grad)
            
            self.entity_emb.trace.clear()
            self.entity_emb.dist_opt.zero_grad()