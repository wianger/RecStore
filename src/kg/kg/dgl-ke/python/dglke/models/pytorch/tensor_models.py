...
class ExternalEmbedding:
    ...
    def debug_xmh_update(self, gpu_id=-1):
        ...
        with th.no_grad():
            for idxs, grads in self.trace:
                ...
                # idx is in CPU, grad is in cuda:X
                grad_sum = (grad_values * grad_values).mean(1)  # shape=[B]
    ...
    def update(self, gpu_id=-1):
        ...
        with th.no_grad():
            for idx, data in self.trace:
                ...
                # if dist.get_rank() == 0:
                #     XLOG.warn(f"original back, rank{dist.get_rank()}, {idx.shape[0]}")
