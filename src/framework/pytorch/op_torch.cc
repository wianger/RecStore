...
torch::Tensor emb_read_torch(const torch::Tensor& keys) {
    ...
    TORCH_CHECK(keys.scalar_type() == torch::kInt64, "Keys tensor must have dtype int64");
    ...
    try {
        ...
        throw std::runtime_error("Error in EmbRead: " + std::string(e.what()));
    }
...
void emb_update_torch(const torch::Tensor& keys, const torch::Tensor& grads) {
    ...
    TORCH_CHECK(grads.scalar_type() == torch::kFloat32, "Grads tensor must have dtype float32");
    TORCH_CHECK(grads.is_contiguous(), "Grads tensor must be contiguous");

    TORCH_CHECK(keys.size(0) == grads.size(0), "Keys and Grads tensors must have the same size in dimension 0");
    TORCH_CHECK(grads.size(1) == base::EMBEDDING_DIMENSION_D, "Grads tensor has wrong embedding dimension");
    ...
    base::RecTensor rec_grads(
        const_cast<void*>(grads.data_ptr()),
        {L, D},
        base::DataType::FLOAT32);
