...
class SpdkWrapper {
 public:
  ...
  virtual void SubmitReadCommand(void *pinned_dst, const int64_t bytes, const int64_t lba,
                                 spdk_nvme_cmd_cb func, void *ctx) = 0;

  virtual int SubmitWriteCommand(const void *pinned_src, const int64_t bytes,
                                 const int64_t lba, spdk_nvme_cmd_cb func, void *ctx) = 0;

  virtual void SyncRead(void *pinned_dst, const int64_t bytes, const int64_t lba) = 0;
  
  virtual void SyncWrite(const void *pinned_src, const int64_t bytes,
                         const int64_t lba) = 0;

  virtual void Sync2Read(void *pinned_dst, const int64_t lba) =0;

  virtual void PollCompleteQueue() = 0;
