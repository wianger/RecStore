...
- For current PET staging-copy capacity runs, keep `--prefetch-depth=16` with `--rdma-rc-qps-per-client-per-shard=16` as the stable default. A 2026-05-31 prefetch sweep at `qps=16`, `batch_keys=500` measured about `24.69M/32.66M/39.29M/32.18M keys/s` for depths `8/12/16/24`; later `qps=20` runs reached up to about `46.69M keys/s` but were not stable enough to become the default.
- Treat larger `batch_keys` as request-size capacity observations, not a framework-level optimization direction, unless the user explicitly asks for batch-size tuning.
- In `DramValueStore::ReadFlatFixedRows`, future-row software prefetch was a negative experiment. The retained value-store micro-optimization is generic power-of-two shift row addressing; the older 512B-only branch was replaced so fixed-size workloads do not need a special-case fallback.
...
- As of `2026-06-01`, round-robin QP acquisition, client wait-spin, more client processes, and deeper single-client outstanding via `slots_per_qp=2` were negative experiments. Keep them out of default tuning.
