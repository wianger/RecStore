...
class RecTensor {
public:
    RecTensor() : data_ptr_(nullptr), dtype_(DataType::UNKNOWN), num_elements_(0) {}
    ...
    T* data_as() const {
        if (dtype_ == DataType::UINT64 && !std::is_same<T, uint64_t>::value) {
            throw std::runtime_error("Type mismatch: Tensor is UINT64, accessed as different type.");
        }
        if (dtype_ == DataType::FLOAT32 && !std::is_same<T, float>::value) {
            throw std::runtime_error("Type mismatch: Tensor is FLOAT32, accessed as different type.");
        }
        if (dtype_ == DataType::FLOAT16 && !std::is_same<T, c10::float16_t>::value) {
            throw std::runtime_error("Type mismatch: Tensor is FLOAT16, accessed as different type.");
        }
        if (dtype_ == DataType::INT32 && !std::is_same<T, int32_t>::value) {
            throw std::runtime_error("Type mismatch: Tensor is INT32, accessed as different type.");
        }
        if (dtype_ == DataType::INT16 && !std::is_same<T, int16_t>::value) {
            throw std::runtime_error("Type mismatch: Tensor is INT16, accessed as different type.");
        }
        if (dtype_ == DataType::INT8 && !std::is_same<T, int8_t>::value) {
            throw std::runtime_error("Type mismatch: Tensor is INT8, accessed as different type.");
        }
    ...
    int64_t shape(size_t i) const {
        if (i >= shape_.size()) throw std::out_of_range("Shape index out of range.");
        return shape_[i];
    }
    ...
    void recalculate_num_elements() {
        if (shape_.empty()) {
            ...
            for(long long dim_size : shape_) {
                if (dim_size < 0) throw std::runtime_error("Tensor dimension size cannot be negative.");
            }
